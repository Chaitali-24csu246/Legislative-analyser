...
def call_llm(prompt: str, temperature: float) -> str:
    # Try Ollama with a hard 45 second timeout
    def _ollama():
    ...
    try:
        with concurrent.futures.ThreadPoolExecutor(max_workers=1) as ex:
            future = ex.submit(_ollama)
            result = future.result(timeout=45)  # hard kill after 45s
            if result and not result.startswith("[LLM ERROR"):
                return result
    except concurrent.futures.TimeoutError:
        st.toast("Ollama timed out — switching to HuggingFace API...", icon="⚡")
    except Exception:
        pass
    ...
    try:
        ...
        if not hf_token:
            return "[LLM ERROR: No HF_TOKEN set and Ollama unavailable]"
        ...
        return f"[LLM ERROR: Both Ollama and HF fallback failed — {str(e)}]"
# ══════════════════════════════════════════════
# Helpers
# ══════════════════════════════════════════════
